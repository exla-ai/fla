...
def create_torch_dataset(
    ...
    if len(all_repo_ids) > 1:
        ...
        for rid in all_repo_ids:
            ...
            # If repo_id_to_prompt is available, wrap dataset with prompt injection
            repo_id_to_prompt = getattr(data_config, 'repo_id_to_prompt', None)
            if repo_id_to_prompt and rid in repo_id_to_prompt:
                prompt = repo_id_to_prompt[rid]
                dataset = TransformedDataset(dataset, [_transforms.InjectDefaultPrompt(prompt)])
                logging.info(f"  Injected prompt for {rid}: '{prompt}'")
